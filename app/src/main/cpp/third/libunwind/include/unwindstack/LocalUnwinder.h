...
// Forward declarations.
    ...
    struct LocalFrameData {
        LocalFrameData(MapInfo *map_info, uint64_t pc, uint64_t rel_pc,
...
// as fast as possible.
    ...
    public:
        ...
        LocalUnwinder(const std::vector<std::string> &skip_libraries) : skip_libraries_(
                skip_libraries) {}

        ~LocalUnwinder() = default;

        MapInfo *FindMaps(uint64_t p);
